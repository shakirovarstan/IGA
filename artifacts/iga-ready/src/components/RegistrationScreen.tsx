      ...
      <div className="w-full max-w-md pt-12 px-6 flex flex-col flex-1">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] p-6 shadow-sm border border-[var(--card-border)] mb-8 flex-1">
          <div className="space-y-4">
            ...
            <div className="relative">
              <label className="absolute -top-2 left-4 bg-white px-1 text-[10px] font-black tracking-wider uppercase text-[var(--text-muted)] z-10">Класс</label>
              <select
                value={grade}
                onChange={e => setGrade(e.target.value)}
                className="w-full h-14 px-4 rounded-[1.25rem] border-2 border-[var(--card-border)] bg-transparent focus:border-[var(--primary)] outline-none text-[var(--text-main)] font-bold appearance-none"
              >
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                <svg width="14" height="8" viewBox="0 0 14 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M2 2L7 6L12 2" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            </div>
            <div className="relative">
              <label className="absolute -top-2 left-4 bg-white px-1 text-[10px] font-black tracking-wider uppercase text-[var(--text-muted)] z-10">Телефон</label>
              <div className="flex items-center h-14 px-4 rounded-[1.25rem] border-2 border-[var(--card-border)] bg-transparent focus-within:border-[var(--primary)] transition-colors">
                <select 
                  value={countryCode} 
                  onChange={e => setCountryCode(e.target.value)}
                  className="bg-transparent text-[var(--text-main)] font-semibold mr-2 outline-none cursor-pointer appearance-none"
                >
                  <option value="+996">+996</option>
                  <option value="+7">+7</option>
                </select>
                <div className="w-[1px] h-6 bg-[var(--card-border)] mx-2"></div>
                <input
                  type="text"
                  value={phone}
                  onChange={e => setPhone(e.target.value)}
                  placeholder="(000) 000-00-00"
                  className="flex-1 h-full bg-transparent outline-none text-[var(--text-main)] font-semibold placeholder:text-slate-300 placeholder:font-normal"
                />
              </div>
            </div>
          </div>
          ...
          <div className="mt-8">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-[10px] uppercase font-black text-[var(--primary)] tracking-wider">Шаг 1 из 3</span>
            </div>
        ...
        <div className="flex items-center justify-center gap-6 mt-auto pb-6 text-slate-500">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 relative -top-0.5" />
            <span className="text-sm font-medium">Одобрено <b className="text-slate-600">IGA</b></span>
          </div>
          <div className="flex items-center gap-2">
            <Users className="w-5 h-5 relative -top-0.5" />
            <span className="text-sm font-medium">50,000+ Учеников</span>
          </div>
        </div>
      </div>
    </div>
